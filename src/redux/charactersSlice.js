...
export const fetchCharacters = createAsyncThunk(
  ...
  async (page) => {
    const res = await axios(
      ...
      }/characters?limit=${removeSkyler}&offset=${page * char}`
    );
    return res.data;
  }
);
...
const charactersSlice = createSlice({
  ...
  extraReducers: {
    ...
    [fetchCharacters.fulfilled]: (state, action) => {
      ...
      const filtredCharacters = action.payload.filter(
        (character) => character.name !== "Skyler White"
      );
